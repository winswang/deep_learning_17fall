...
class Visualizer:
    ...
    def show_sequences(self,seq1,seq2,**kwargs):
        ...
        # gather labels if input
        labels = []
        if 'labels' in kwargs:
            labels = kwargs['labels']
            
            # plot sequence 1 
            ax.plot(np.arange(len(seq1)),seq1,c = [1,0.8,0.5],zorder = 1,linewidth=3.5,label = labels[0])
            ...
            # plot sequence 2
            if np.size(seq2) > 0:
                ax.plot(np.arange(len(seq2)),seq2,c = [0.5,0.7,1],zorder = 2,linewidth=2.5,label = labels[1])
